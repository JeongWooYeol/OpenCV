...
img_gray = cv2.cvtColor(mean, cv2.COLOR_BGR2GRAY)
img_gray = cv2.GaussianBlur(img_gray, (3,3), 0)
_, th = cv2.threshold(img_gray, 0, 255, cv2.THRESH_BINARY | cv2.THRESH_OTSU)
cv2.imshow('thresh', th)

dst = cv2.distanceTransform(th, cv2.DIST_L2, 3)
...
ret, bg_th = cv2.threshold(dst, 0.3*dst.max(),  255, cv2.THRESH_BINARY_INV)
